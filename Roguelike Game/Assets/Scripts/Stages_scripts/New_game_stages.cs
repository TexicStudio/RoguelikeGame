...
public class New_game_stages : MonoBehaviour
{
    ...
    [Space]
    public GameObject add_basic_map_gameObject;
    public Button add_basic_map;
    ...
    [Space]
    [Header("   список подземелей")]
    public GameObject dungeon_map_list_container;

    [Space]
    public GameObject add_dungeon_map_gameObject;
    public Button add_dungeon_map;
    ...
    [Space]
    [Header("   Кнопка Сгенерировать Историю")]
    ...
    [Space]
    public int max_dungeon_map = 0;
    public int max_basic_map = 0;
    ...
    public void Add_map_Click(string map_type)
    {
        ...
        {
            case "basic":
                if (basic_map_panel_list.Count < max_basic_map)
                {
                    start_add = true;
                ...
                break;
            case "dungeon":
                if (dungeon_map_panel_list.Count < max_dungeon_map)
                {
                    start_add = true;
        ...
        if (start_add == true)
        {
            ...
            if (paths.Length > 0)
            {
                if (File.Exists(paths[0]))
                {
                    ...
                    {
                        case "basic":
                            Add_new_basic_map(paths[0]);
                            break;
                        case "dungeon":
                            Add_new_dungeon_map(paths[0]);
                            break;
                    }
                }
                else
    ...
    private void Add_new_basic_map(string new_path)
    {
        GameObject new_map_panel_gameObject = Instantiate(Resources.Load("Prefab/Interface/New_game/new_game_map_panel", typeof(GameObject))) as GameObject;
        new_map_panel_gameObject.transform.SetParent(basic_map_list_container.transform);
        new_map_panel_gameObject.transform.SetAsLastSibling();

        add_basic_map_gameObject.transform.SetAsLastSibling();

        new_map_panel_gameObject.name = "" + basic_map_panel_list.Count + "_" + new_path;

        new_game_map_panel_script new_map_panel = new_map_panel_gameObject.GetComponent<new_game_map_panel_script>();
        new_map_panel.id = basic_map_panel_list.Count;
        new_map_panel.map_type = basic_map_type;
        new_map_panel.path = new_path;

        new_map_panel.name_text.text = "" + new_path;

        new_map_panel.button_delete.onClick.AddListener(() => Delete_map(new_map_panel.id, new_map_panel.map_type));

        basic_map_panel_list.Add(new_map_panel);
    }

    private void Add_new_dungeon_map(string new_path)
    {
        GameObject new_map_panel_gameObject = Instantiate(Resources.Load("Prefab/Interface/New_game/new_game_map_panel", typeof(GameObject))) as GameObject;
        new_map_panel_gameObject.transform.SetParent(dungeon_map_list_container.transform);
        new_map_panel_gameObject.transform.SetAsLastSibling();

        add_dungeon_map_gameObject.transform.SetAsLastSibling();

        new_map_panel_gameObject.name = "" + dungeon_map_panel_list.Count + "_" + new_path;
        ...
        dungeon_map_panel_list.Add(new_map_panel);
    }

    private void Delete_map(int new_id, string new_type)
    {
        switch (new_type)
        {
            case "basic":
                basic_map_panel_list[new_id].Delete_map();
                basic_map_panel_list.RemoveAt(new_id);

                for (int new_i = new_id; new_i < basic_map_panel_list.Count; new_i++)
                {
                    basic_map_panel_list[new_i].id = new_i;
                }
                break;
            case "dungeon":
                dungeon_map_panel_list[new_id].Delete_map();
                dungeon_map_panel_list.RemoveAt(new_id);

                for (int new_i = new_id; new_i < dungeon_map_panel_list.Count; new_i++)
                {
                    dungeon_map_panel_list[new_i].id = new_i;
                }
                break;
        }
    }
